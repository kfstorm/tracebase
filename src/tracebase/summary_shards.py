...
class ShardObservability:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "failed": self.failed,
            "protocol_errors": list(self.errors),
        }
...
def inspect_shards(
    ...
    try:
        ...
        return ShardObservability(0, 0, 0, 1, (f"could not read {notes_path}",))

    try:
        data = _status_data(notes)
    except ValueError as error:
        return ShardObservability(0, 0, 0, 1, (str(error),))
    if not isinstance(data, dict) or not isinstance(data.get("shards"), list):
        return ShardObservability(0, 0, 0, 1, ("SHARD_STATUS.shards must be a list",))
    if context_dir is not None and not data["shards"]:
        ...
        if has_context:
            return ShardObservability(
                0, 0, 0, 1, ("non-empty Context has no declared shards",)
            )
        ...
        if report != expected_report:
            ...
            if report_path.is_symlink() or not report_path.is_file():
                ...
                try:
                    ...
                    entry_failed = True
        if status == "complete" and not entry_failed:
            completed += 1
        if entry_failed:
            failed += 1

    return ShardObservability(
        len(data["shards"]), completed, retried, failed, tuple(errors)
    )
