...
def complete_personal_shard_sections(work_dir: Path) -> None:
    """Add an omitted empty context section to personal shard reports."""
    try:
        notes = (work_dir / "NOTES.md").read_text(encoding="utf-8")
        data = _status_data(notes)
    ...
    for item in data["shards"]:
        if not isinstance(item, dict):
            continue
        if item.get("attribution_modes") != [AttributionMode.PERSONAL.value]:
            continue
        report = item.get("report")
        shard_id = item.get("id")
        if not isinstance(report, str) or not isinstance(shard_id, str):
            continue
        if report != f"/work/shards/{shard_id}.md":
            continue
        report_path = work_dir / "shards" / f"{shard_id}.md"
        if report_path.is_symlink() or not report_path.is_file():
            continue
        try:
            ...
            continue
        if (
            _USER_WORK_SECTION in report_text
            and _CONTEXT_ONLY_SECTION not in report_text
        ):
            try:
